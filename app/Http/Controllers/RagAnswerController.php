...
class RagAnswerController extends Controller
{
    public function answer(Request $request)
    {
        ...
        if ($mode === 'quote' || ($mode === 'auto' && $this->hasQuoteIntent($query))) {
            ...
            $quote  = $this->ensureDoubleQuoted($quote);
            $quote  = $this->ensureDoubleQuoted($quote);
            $final  = $this->formatAs($quote, $format);
        ...
        if ($mode === 'summary' || ($mode === 'auto' && $this->hasSummaryIntent($query))) {
            ...
            // >>> GUARD específico do SUMMARY: garante 1 bullet + ≥120 chars + keywords
            if (trim((string)$final) === '') {
                $plain = trim(preg_replace('/\s+/', ' ', $this->combineChunksForList($neighbors ?? [])));
                if ($plain === '') {
                    $plain = trim(preg_replace('/\s+/', ' ', (string)($query ?? 'Resumo solicitado.')));
                }
                $para  = mb_substr($plain, 0, 600, 'UTF-8');
...
// Guard: se o resumo final ficar vazio, gera 1 bullet + parágrafo >=120 chars + keywords
if (trim((string)$final) === '') {
    $plain = trim(preg_replace('/\s+/', ' ', $this->combineChunksForList($neighbors ?? [])));
    if ($plain === '') {
        $plain = trim(preg_replace('/\s+/', ' ', (string)($query ?? 'Resumo solicitado.')));
